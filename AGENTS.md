...
bun run fleet -- serve|sessions|projects|herd|dialog-reply|pty-spawn|pty-list|pty-kill|spawn|add-repo|add|provision|stop|remove|rm-project|add-worktree|rm-worktree|prompt
...
| `fleet/server.ts` | Control plane (default `:4722`, bind `config.host`): `/ctl/*` including herd, dialog/reply, pty, push, telegram webhook, projects/worktrees; auth on every fetch |
| `fleet/auth.ts` | Fleet operator auth + CSRF (`authorizeFleetRequest`, `assertFleetBindSafe`) |
| `fleet/projector.ts` | In-memory herd snapshot for `GET /ctl/herd` (no tokens/endpoints) |
| `fleet/dialog-reply.ts` | Epoch-scoped dialog parse/resolve + idempotency |
| `fleet/observer.ts` | Connector tap + `/events?role=observer` dial; does not pin daemon idle |
| `fleet/notifications/` | WebPush (VAPID) + Telegram dispatchers and redaction |
| `fleet/pty-supervisor.ts` | Isolated non-OMP process runner (`/ctl/pty*`) |
...
| `src/pwa.ts` | Service worker register, Web Push subscribe, `?daemon=` / notificationclick attach |
| `public/` | PWA assets copied to `dist/`: `manifest.webmanifest`, `icon.svg`, `sw.js` |
...
| `docs/` | `architecture.md` (wire + module map), `position.md` (product vs TUI/`omp-gui`), `CONVERGENCE_BLUEPRINT.md` (as-built Phases 1–4), `HERDR_COLLIE_OMP_ANALYSIS.md`, `STATUS.md`, `release.md`. `docs/research/` is named in older notes and is not in this tree. |
...
- Idle auto-exit (default `--idle-timeout 30m`): no attached *non-observer* clients, no running agent/queue, no in-flight bash/eval, no open dialog, no live collab room. `GET /events?role=observer` does not count. `0` disables. The `.jsonl` is durable; fleet marks the entry `asleep` and respawns with `--resume`.
...
- **Fleet operator auth.** Every fleet `fetch` (including `/events` and `/command`) runs `authorizeFleetRequest`. Off-loopback needs `OMP_FLEET_TOKEN` / `--token` or `--tailscale-auth` with `Tailscale-User-Login` from `100.64/10`. Non-loopback bind without those is a startup error. CSRF: `x-omp-csrf`, same-origin, or loopback-to-loopback Origin (vite). Loopback CLI with no Origin is allowed. `/ctl/herd` never serializes tokens/endpoints.
- **Observer vs retain.** A retained *browser* connector stream still counts as an attached client (idle suspended). The dedicated observer stream and `SseConsumer.observer` do not.
- **PTY workers** are not omp-session children. They do not speak `OMP_SESSION|` and must not go through spawn templates.
...
- Module layout: logic/data modules live in feature dirs, `src/sprites/` (pixel-art sprite engine + character data), `src/prompt/` (slash commands, autocomplete, prompt history, danger-confirm), `src/text/` (markdown, diff, clipboard, images), `src/usage/` (context thresholds, usage rows, model-options), `src/fleet-ui/` (dir-picker browse model, daemon uptime + fleet-debug parsing), `src/prefs/` (client theme + settings-panel display helpers), `src/chat/` (tool-run grouping). `src/store/` holds the per-domain action modules behind the `state.ts` facade; `src/components/` is presentational only. `src/pwa.ts` is the SW/push/deep-link bootstrap (called from `src/index.tsx`).
- Styling: one global stylesheet split by domain, `src/styles/` holds `tokens.css` (design tokens + `:root[data-theme=…]` palettes), `base.css` (reset, `.icon`, global button/focus, pointer:coarse hit-targets, badge geometry, safe-area padding), `app.css` (shell + status bar + `100dvh`), `chat.css` (stream/markdown), `tools.css` (tool/bash renderers), `prompt.css` (composer/autocomplete), `modals.css` (dialogs/pickers/ask), `settings.css`, `fleet.css` (sidebar/roster/projects/debug; sidebar-toggle safe-area offset), `usage.css` (meters/breakdown), plus `src/tx/tx.css` co-located with the transcripts view. `src/styles.css` is the `@import` entry; import order encodes the original cascade and is load-bearing: new files/rules go in a position that preserves it. Plain kebab-case feature-prefixed classes (`msg-*`, `tool-*`, `bash-*`, `settings-*`, `daemon-*`…), design tokens as CSS custom properties in `:root` (`--space-1..7`, `--r-*`, `--fs-*`, `--z-*`), theme palettes override only `--*` vars. No CSS modules, no inline styles except rare one-offs.
- Dev handle: `window.__ompState` (DEV only). No router; modals keyed off `state.modal === …`. `?daemon=` is handled in `startPwa()`, not a router.

### Tests
- `bun:test` across two trees: **co-located** `*.test.ts` next to source (`src/state.test.ts` ↔ `src/state.ts`, etc.), plus the `test/` stats suite (14 files) covering the `fleet/stats/` API. Run through `bun scripts/test.ts`, which pins `--parallel` to the machine's **physical** core count (logical-count workers oversubscribe HT/hybrid boxes and thrash the daemon-spawning suites), `--timeout 15000`, `--retry 0`. Extra args forward to `bun test`.
...
- **Heavy (spawn real daemons/processes)**: `server/omp-session.test.ts`, `server/collab-integration.test.ts`, `fleet/integration.test.ts` (real fleet + 3 real omp-session daemons, serial, hermetic `PI_CODING_AGENT_DIR` → no model → prompts fail with `ok:false` by design), `fleet/supervisor.test.ts` (fake `sh` children), `fleet/server-*.test.ts`, `fleet/edge-*.test.ts`, `fleet/pty-supervisor.test.ts` (real `Bun.spawn`). **Light/pure**: `src/**/*.test.ts` (state tested against a `FakeEventSource` + stubbed fetch), `shared/sse.test.ts`, `fleet/spawn-parse/selectors/events/daemons-aggregator/config/discovery/registry/fanout/connector.test.ts`, `fleet/auth.test.ts`, `fleet/projector.test.ts`, `fleet/dialog-reply.test.ts`, `fleet/observer.test.ts`, `fleet/notifications/dispatcher.test.ts`, `server/settings-model/collab-relay/collab-host.test.ts`.
...
- `dist/`, `node_modules/` are gitignored; `docs/architecture.md`, `docs/position.md`, `docs/CONVERGENCE_BLUEPRINT.md`, `docs/HERDR_COLLIE_OMP_ANALYSIS.md`, `docs/STATUS.md`, `docs/release.md` are committed docs: update, don't delete. `docs/research/` is named in older notes and is not in this tree.
